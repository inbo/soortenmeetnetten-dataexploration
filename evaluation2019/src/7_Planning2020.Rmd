...
steekproef_vw_orig <- read_sf("../../data/steekproefkader/variabele_selectie._dissolve.shp") %>%
   st_drop_geometry()

steekproefkader_vw_orig <- read_sf("../../data/steekproefkader/variabele_steekproefkader_dissolve.shp", crs = 31370) 
...
steekproefkader_vw_update <- steekproefkader_vw %>%
  ...
  rename(deelgebied = Deelgebied, zoekzone = Zoekzone, grts_ranking = GRTS, naam = toponiem, steekproef = Steekproef)

nieuwe_locaties <- steekproefkader_vw_update %>%
  filter(naam %in% c("Coupure Avelgem", "Oudbroekpolder"))

st_write(steekproefkader_vw, "../../data/steekproefkader", "variabele-waterjuffer_steekproefkader_versie2020-03-10", driver = "ESRI Shapefile", update = TRUE)
...
steekproef_vg_orig <- read_sf("../../data/steekproefkader/VroegeGlazenmaker_SteekproefGebieden.shp") %>%
   st_drop_geometry() %>%
  rename(GRTS = GRTS_gbd) %>%
  mutate(Steekproef = 1)

steekproefkader_vg_orig <- read_sf("../../data/steekproefkader/vroege_steekproefkader_dissolve.shp", crs = 31370) 
...
steekproefkader_vg_update <- steekproefkader_vg %>%
  mutate(Steekproef = ifelse(toponiem %in% c("Grootbroek"), 0, Steekproef),
         Steekproef = ifelse(Zoekzone %in% c("Z0006"), 1, Steekproef),
         toponiem = ifelse(Zoekzone == "Z0006", "Zoete waters", toponiem)) %>%
  select(deelgebied = Deelgebied, zoekzone = Zoekzone, grts_ranking = GRTS, naam = toponiem, steekproef = Steekproef)

nieuwe_locaties <- steekproefkader_vg_update %>%
  filter(naam %in% c("Zoete waters"))

st_write(steekproefkader_vw, "../../data/steekproefkader", "vroege-glazenmaker_steekproefkader_versie2020-03-10", driver = "ESRI Shapefile", update = TRUE)

st_write(nieuwe_locaties, "../../output/update_meetnetten", "vroege-glazenmaker_addLocations_2020-03-10", driver = "ESRI Shapefile", update = TRUE)
```
