...
aantallen <- get_counts_smp()  %>%
  filter(activiteit != "nest (hervondst)")
...
provinces_flanders <- read_sf(file.path(fileman_up("soortenmeetnetten-queries"), "gis/provincies"), "Provincies2015", crs = 31370) %>%
...
locaties_provincies <- get_locations_smp(only_active = FALSE) %>%
  ...
  st_drop_geometry() %>%
  select(meetnet, locatie, provincie_locatie = provincie, is_active) 

locaties_provincies_active <- locaties_provincies %>%
  filter(is_active)

locaties_provincies <- locaties_provincies %>%
  select(-is_active)
...
users_provincie <- read_vc(root = fileman_up("soortenmeetnetten-queries"),file =  "raw/meetnetten_users") %>%
