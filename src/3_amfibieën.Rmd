...
overzicht_aantallen_levensstadium_primair_wide %>%
  filter(soortgroep == sg) %>%
   filter(soort_nl != "Meerkikker" | is.na(soort_nl)) %>%
  mutate(soort_nl = ifelse(soort_wet == "Pelophylax lessonae/ kl. esculentus", "Poelkikker/Bastaardkikker", soort_nl)) %>%
  select(-soortgroep, -meetnet, "Nederlandse naam" = soort_nl, "wetenschappelijke naam" = soort_wet) %>%
  kable() %>%
...
```{r boomkikkerMean, fig.cap= str_c("Evolutie gemiddelde getelde aantallen per bezoek: ", soort), fig.height= 3, fig.width= 10}
