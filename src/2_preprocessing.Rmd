...
# n_locaties per provincie

locaties_provincies_overzicht <- locaties_provincies_active %>%
  group_by(provincie_locatie, meetnet) %>%
  summarise(n_telgebieden_totaal = n_distinct(locatie)) %>%
  ungroup()


# details per provincie
overzicht_bezoeken_tellers_provincie_locatie_detail <- tellers_provincies %>%
  semi_join(locaties_provincies_active, by = c("meetnet", "locatie")) %>%
  filter(jaar >= 2016) %>%
...
overzicht_bezoeken_tellers_provincie_locatie_detail %>%
  filter(jaar == year_evaluation) %>%
  filter(!is.na(provincie_locatie)) %>%
  left_join(locaties_provincies_overzicht, by = c("provincie_locatie", "meetnet")) %>%
  select(provincie = provincie_locatie, jaar, soortgroep, meetnet, aantal_tellocaties_totaal = n_telgebieden_totaal, aantal_tellers = n_tellers, aantal_tellingen =  n_tellingen, aantal_tellocaties_geteld = n_telgebieden) %>%
  write_excel_csv2(str_c("../output/monitoringinspanning_meetnet_provincielocatie_", year_evaluation, ".csv"))
...
overzicht_provincieteller <- tellers_provincies %>%
  filter(jaar >= 2016) %>%
  group_by(provincie_teller, jaar) %>%
  summarise(n_tellers = n_distinct(naam_teller),
            n_telgebieden = n_distinct(locatie)) %>%
  ungroup() %>% 
  select(provincie_teller, everything()) %>%
...
overzicht_provincielocatie <- tellers_provincies %>%
  filter(jaar >= 2016) %>%
  group_by(provincie_locatie, jaar) %>%
  summarise(n_tellers = n_distinct(naam_teller),
            n_tellingen = n_distinct(visit_id),
            n_telgebieden = n_distinct(locatie)) %>%
  ungroup() %>% 
...
overzicht_provincielocatie %>%
  filter(jaar == year_evaluation) %>%
  filter(!is.na(provincie_locatie)) %>%
  rename( aantal_tellers = n_tellers, aantal_tellingen =  n_tellingen, aantal_tellocaties_geteld = n_telgebieden) %>%
  write_excel_csv2(str_c("../output/monitoringinspanning_totaal_provincielocatie_", year_evaluation, ".csv"))
...
write.csv2(overzicht_bezoeken_tellers_totaal_long,
           str_c("../output/monitoringsinspanning_vlaanderen_", versie, ".csv"), row.names = FALSE)

overzicht_bezoeken_tellers_totaal_long %>%
  filter(jaar == year_evaluation) %>%
  write_excel_csv2(str_c("../output/monitoringinspanning_totaal_vlaanderen_", year_evaluation, ".csv"))
```
...
Een tweede figuur geeft de evolutie van de getelde aantallen binnen het seizoen weer en dit voor elk jaar waarin er geteld werd. Onderstaand voorbeeld toont het getelde aantal individuen van de Kommavlinder. De stippelijnen verbinden tellingen van eenzelfde locatie. Op basis hiervan krijg je een idee hoe de getelde aantallen binnen een telseizoen variëren.     
